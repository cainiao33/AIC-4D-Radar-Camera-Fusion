...
class KittiDataset(Dataset):
    def __init__(self, configs, mode='train', lidar_aug=None, hflip_prob=None, num_samples=None):
        ...
        train_subdir = getattr(configs, 'train_subdir', 'training')
        val_subdir = getattr(configs, 'val_subdir', train_subdir)
        test_subdir = getattr(configs, 'test_subdir', 'testing')
        if self.mode == 'train':
            sub_folder = train_subdir
        elif self.mode == 'val':
            sub_folder = val_subdir
        else:
            sub_folder = test_subdir
        ...
        use_imagesets = getattr(configs, 'use_imagesets', True)
        split_txt_path = None
        if use_imagesets:
            split_txt_candidates = []
            imagesets_dir = getattr(configs, 'imagesets_dir', None)
            if imagesets_dir is not None:
                candidate_path = os.path.join(imagesets_dir, f'{mode}.txt')
                split_txt_candidates.append(candidate_path)
            default_path = os.path.join(self.dataset_dir, 'ImageSets', f'{mode}.txt')
            split_txt_candidates.append(default_path)
            split_txt_path = next((os.path.normpath(p) for p in split_txt_candidates if Path(os.path.abspath(os.path.normpath(p))).exists()), None)

        if split_txt_path is not None:
            with open(split_txt_path, 'r') as split_file:
                self.sample_id_list = [int(x.strip()) for x in split_file.readlines() if x.strip()]
        else:
            if not self.is_test and os.path.isdir(self.label_dir):
                id_source_dir = self.label_dir
                ext = '.txt'
            else:
                id_source_dir = self.lidar_dir
                ext = '.bin'
            if not os.path.isdir(id_source_dir):
                raise FileNotFoundError(f"Cannot find samples for mode '{mode}' in '{id_source_dir}'")
            self.sample_id_list = sorted([int(os.path.splitext(f)[0]) for f in os.listdir(id_source_dir) if f.endswith(ext)])
    ...
    def get_lidar(self, idx):
        ...
        return read_lidar_file_with_fallback(lidar_file)
