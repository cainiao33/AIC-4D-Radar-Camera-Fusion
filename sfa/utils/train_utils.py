...
def save_checkpoint(checkpoints_dir, saved_fn, model_state_dict, utils_state_dict, epoch, is_best=False, best_tag='best'):
    """Save checkpoint every epoch and optionally track the best model"""
    ...
    # Use legacy serialization to avoid Windows zip writer issues
    torch.save(model_state_dict, model_save_path, _use_new_zipfile_serialization=False)
    torch.save(utils_state_dict, utils_save_path, _use_new_zipfile_serialization=False)

    print('save a checkpoint at {}'.format(model_save_path))

    if is_best:
        best_model_path = os.path.join(checkpoints_dir, 'Model_{}_{}.pth'.format(saved_fn, best_tag))
        best_utils_path = os.path.join(checkpoints_dir, 'Utils_{}_{}.pth'.format(saved_fn, best_tag))
        torch.save(model_state_dict, best_model_path, _use_new_zipfile_serialization=False)
        torch.save(utils_state_dict, best_utils_path, _use_new_zipfile_serialization=False)
        print('update best model at {}'.format(best_model_path))
