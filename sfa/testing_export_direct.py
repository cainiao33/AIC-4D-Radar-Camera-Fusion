"""
在 DRadDataset/testing 上进行批量推理，不依赖 ImageSets。输出 KITTI 格式的检测结果。
"""
...
def parse_configs():
    parser = argparse.ArgumentParser(description='Direct inference on testing split (no ImageSets)')
    ...
    parser.add_argument('--output-dir', type=str, default=None, metavar='PATH',
                        help='Directory to store KITTI txt outputs (default results/<saved_fn>/testing_export)')
    ...
    if cfg.output_dir is None:
        cfg.output_dir = os.path.join('.', '激进P')
...
def main():
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(dataloader):
            ...
            detections = post_processing(detections, configs.num_classes, configs.down_ratio, configs.peak_thresh,
                                         inter_class_nms=True, nms_thresh=0.3)
    ...
    print(f"\nFinished {processed} samples in {elapsed:.2f}s ({processed / max(elapsed, 1e-9):.2f} FPS)")
