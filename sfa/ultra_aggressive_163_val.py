"""
Ultra-aggressive推理脚本，用于163轮次模型在验证集上的评估
...
def parse_configs():
    parser = argparse.ArgumentParser(description='Ultra-aggressive 163-epoch model inference on validation set')
    parser.add_argument('--saved_fn', type=str, default='ultra_aggressive_163_val', metavar='FN',
    ...
    parser.add_argument('--test-subdir', type=str, default='split/val', metavar='DIR',
    ...
    cfg.train_subdir = cfg.test_subdir
...
def print_inference_summary(processed_samples, total_detections, elapsed_time, output_dir):
    """打印推理总结"""
    print(f"\n{'='*80}")
    print("163轮模型超激进验证集推理完成!")
    print('='*80)
    print(f"处理样本数: {processed_samples}")
    print(f"处理时间: {elapsed_time:.2f}s")
    print(f"平均速度: {processed_samples / max(elapsed_time, 1e-9):.2f} FPS")
    print(f"总检测数: {total_detections}")
    print(f"平均检测数: {total_detections / max(processed_samples, 1):.2f}/样本")
    print(f"输出目录: {output_dir}")
    print('='*80)

    # 推测性能指标
    if total_detections > 0:
        avg_detections = total_detections / processed_samples
        print(f"\n性能推测:")
...
def main():
    ...
    print(f'验证集: {configs.test_subdir}')
    ...
    # Print summary
    print_inference_summary(processed, total_detections, elapsed, configs.output_dir)

    # Save inference statistics
    stats_file = os.path.join(configs.output_dir, 'inference_stats.json')
    import json
    stats_data = {
        'model': '163-epoch',
