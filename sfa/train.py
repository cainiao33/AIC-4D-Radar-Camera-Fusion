"""
主训练代码
通过 process_8d_lidar_data_scheme_B() 读取维度 [0,1,2,4] (P映射为intensity)
...
def main_worker(gpu_idx, configs):
    ...
    # Create dataloader
    # 注意: sfa/train.py 对于8D数据就是直接读取0,1,2,4维度，不会进入到5D和4D的处理逻辑中。
    # 数据流是:
    ...
    for epoch in range(configs.start_epoch, configs.num_epochs + 1):
        ...
        if (not configs.no_val) and (epoch % configs.checkpoint_freq == 0):
            # 注意: 验证数据同样遵循8D数据的处理逻辑
            # 数据流: 8D数据 [x,y,z,D,P,R,A,E] → 读取 [0,1,2,4] 维度 → P映射为intensity → 输出4D [x,y,z,intensity]
            val_dataloader = create_val_dataloader(configs)
            if logger is not None:
                logger.info('number of batches in val_dataloader: {}'.format(len(val_dataloader)))
            else:
            ...
            if val_loss < best_val_loss:
                ...
                is_best = True
                if logger is not None:
                    logger.info('New best val_loss: {:.4e} at epoch {}'.format(best_val_loss, epoch))

        # Save checkpoint
        if configs.is_master_node and ((epoch % configs.checkpoint_freq) == 0):
            model_state_dict, utils_state_dict = get_saved_state(model, optimizer, lr_scheduler, epoch, configs)
            save_checkpoint(configs.checkpoints_dir, configs.saved_fn, model_state_dict, utils_state_dict, epoch, is_best=is_best)
    ...
    if logger is not None and not np.isinf(best_val_loss):
        logger.info('Best val_loss: {:.4e} at epoch {}'.format(best_val_loss, best_epoch))
    if tb_writer is not None:
        tb_writer.close()
    if configs.distributed:
        cleanup()
