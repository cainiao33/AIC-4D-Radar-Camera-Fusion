"""
在 DRadDataset/testing 上进行超激进批量推理，不依赖 ImageSets。
采用更严格的非极大值抑制（NMS）和阈值设置，以最大化精度并最小化假阳性。
针对 DRadDataset - fs 数据集进行了修改。
"""
...
def parse_configs():
    parser = argparse.ArgumentParser(description='Ultra-aggressive NMS inference on testing split')
    ...
    parser.add_argument('--dataset-dir', type=str, default='D:\\bev\\SFA3D-modified\\DRadDataset - fs', metavar='PATH',
    ...
    parser.add_argument('--peak_thresh', type=float, default=0.25,
                        help='Score threshold for decoded peaks (ultra-aggressive: 0.25, default was 0.2)')
    parser.add_argument('--nms_thresh', type=float, default=0.2,
                        help='NMS IoU threshold (ultra-aggressive: 0.2, aggressive was 0.3)')
    parser.add_argument('--output-dir', type=str, default=None, metavar='PATH',
                        help='Directory to store KITTI txt outputs (default results/<saved_fn>/ultra_aggressive)')
    ...
    cfg.train_subdir = cfg.test_subdir  # not used but required by dataset
    cfg.val_subdir = cfg.test_subdir

    if cfg.output_dir is None:
        cfg.output_dir = os.path.join(cfg.root_dir, '超激进P_fs')
...
def main():
    ...
    print('\n' + '='*80)
    print('ULTRA-AGGRESSIVE NMS INFERENCE MODE (DRadDataset - fs)')
    print('='*80)
    print(f'Dataset: {configs.dataset_dir}')
    print(f'NMS Threshold: {configs.nms_thresh} (lower = more aggressive)')
    print(f'Peak Threshold: {configs.peak_thresh} (higher = fewer detections)')
    print(f'Inter-class NMS: Enabled')
    print('='*80 + '\n')
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(dataloader):
            ...
            # Ultra-aggressive NMS with lower threshold
    ...
    avg_detections = total_detections / max(processed, 1)
    print(f"\n{'='*80}")
    print(f"Finished {processed} samples in {elapsed:.2f}s ({processed / max(elapsed, 1e-9):.2f} FPS)")
